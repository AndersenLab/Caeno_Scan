...
def select_og_variants(og_variants, selected_ogs, n_var_og = 1):
    """
    Filter to variants in the selected orthogroups and select n_var_og variants from each orthogroup
    """
    #Group variants the data frame by orthogroup
    # TODO - maker sure the orthogroup column is named correctly in the new file format
    sel_og_var = og_variants[og_variants["Orthogroup"].isin(selected_ogs)]

    #Group the data frame by orthogroup
    og_variants = sel_og_var.groupby("Orthogroup")
...
if __name__ == "__main__":
    ...
    causal_og_vars = select_og_variants(strain_var, sim_ogs, 1)
    print("Selected Causal Variants")
    print(causal_og_vars)

    #Simulate effects for causal variants
    causal_og_vars = simulate_og_effect_gamma(causal_og_vars, n_ogs)
    
    #Write output for trait simulations - just id and effect
    causal_og_vars[["Marker", "EFFECT"]].to_csv("causal_og_vars.txt", sep = " ", index = False, header = False)
